"""
Pygame2 color module for color creation and conversion operations.
"""
import sys
import copy
...
class Color (object):
    """Color (r=255, g=255, b=255, a=255) -> Color
    ...
    def __div__ (self, color):
        vals = [0, 0, 0, 0]
        if color.r != 0: vals[0] = self.r / color.r
        if color.g != 0: vals[1] = self.g / color.g
        if color.b != 0: vals[2] = self.b / color.b
        if color.a != 0: vals[3] = self.a / color.a
        return Color (vals[0], vals[1], vals[2], vals[3])
    ...
    def normalize (self):
        ...
        Returns the RGBA values in a normalized form with the range [0;1]
        """
