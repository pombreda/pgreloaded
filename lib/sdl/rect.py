...
class SDL_Rect (ctypes.Structure):
    """SDL_Rect (x=0, y=0, w=0, h=0) -> SDL_Rect
...
def enclose_points (points, clip=None):
    """enclose_points (points, clip) -> bool, SDL_Rect
...
def has_intersection (recta, rectb):
    """has_intersection (recta, rectb) -> bool
...
def intersect_rect (recta, rectb):
    """intersect_rect (recta, rectb) -> bool, SDL_Rect
    
    Calculates the intersection area of the passed rectangles.
    
    The first return value will be True, if both rectangles intersect, otherwise
    False. If both rectangles intersect, the second return value denotes the 
    intersection area of both rectangles. If there is no intersection, the
    returned SDL_Rect value is undefined.
    """
    if not isinstance (recta, SDL_Rect) or not isinstance (rectb, SDL_Rect):
        raise TypeError ("recta and rectb must be SDL_Rect instances")
    result = SDL_Rect ()
...
def intersect_rect_and_line (rect, x1, y1, x2, y2):
    """intersect_rect_and_line (rect, x1, y1, x2, y2) -> (bool, int, int, int, int)
...
def union_rect (recta, rectb):
    """union_rect (recta, rectb) -> SDL_Rect
