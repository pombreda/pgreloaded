...
class ColorTest (unittest.TestCase):
    ...
    def test_is_rgb_color (self):
        ...
        for v in color_combos ():
            assertTrue (is_rgba_color (v))
        
        for v in rgba_combos ():
            assertFalse (is_rgba_color (v))
    ...
    def test_rgba_argb_to_color (self):
        self.assertEqual (RGBA, rgba_to_color)
        self.assertEqual (ARGB, argb_to_color)
        ...
        for index, val in enumerate (rgba_combos ()):
            ...
            if color.r == color.g == color.b == color.a:
                assertEqual (RGBA(val), color)
                assertEqual (ARGB(val), color)
                continue

            assertEqual (RGBA(val), color, "Failed for '%s'" % val)
            assertNotEqual (ARGB(val), color, "Failed for '0x%.8x'" % val)
        ...
        for index, val in enumerate (argb_combos ()):
            ...
            assertEqual (ARGB(val), color, "Failed for '%s'" % val)
            assertNotEqual (RGBA(val), color, "Failed for '0x%.8x'" % val)

    def test_string_to_color (self):
        assertEqual = self.assertEqual
        assertRaises = self.assertRaises
        for method in (string_to_color, convert_to_color, COLOR):
        ...
        self.assertRaises (TypeError, string_to_color, 0xff000000)
        self.assertRaises (TypeError, string_to_color, Color())
        
    def test_convert_to_color (self):
        self.assertEqual (COLOR, convert_to_color)
        cvals = list(color_combos ())
        ...
        self.assertRaises (ValueError, convert_to_color, self)
